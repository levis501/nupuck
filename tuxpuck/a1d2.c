...
static Uint8 _a1d2_reblit(AIPlayer * player, Uint32 time) {
	switch (player->state) {
...
AIPlayer *a1d2_create(Pad * pad, Puck * puck) {
	...
	player->sdl_image = video_create_png_surface(a1d2_png, NULL);
	...
	player->reblit = _a1d2_reblit;
	player->set_state = _a1d2_set_state;
	player->ready = _ready;
	player->free = _a1d2_free;
