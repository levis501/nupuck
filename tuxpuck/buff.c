...
static Uint8 _buff_reblit(AIPlayer * player, Uint32 time) {
	switch (player->state) {
...
AIPlayer *buff_create(Pad * pad, Puck * puck) {
	...
	player->sdl_image = video_create_png_surface(buff_png, NULL);
	...
	player->reblit = _buff_reblit;
	player->set_state = _buff_set_state;
	player->ready = _ready;
	player->free = _buff_free;
