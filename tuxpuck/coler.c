...
/* externals */
extern unsigned char coler_png[];

/* statics */
static char _name[] = "coler";
/* unused? static Uint8 _serve_state = 0, _is_ready = 1; */
static Uint8 _is_ready = 1;

/* functions */
static void _coler_free(AIPlayer * player) {
	SDL_FreeSurface(player->sdl_image);
	free(player);
}

static Uint8 _coler_reblit(AIPlayer * player, Uint32 time) {
	switch (player->state) {
...
/* unused?
static void _coler_serve(AIPlayer * player, Uint32 time) {
...
static void _coler_set_state(AIPlayer * player, Uint8 state) {
	player->state = state;
...
AIPlayer *coler_create(Pad * pad, Puck * puck) {
	...
	player->sdl_image = video_create_png_surface(coler_png, NULL);
	...
	player->puck = puck;
	player->speed = 1.3;
	player->hit_power = 1.5;
	player->state = PLAYER_STATE_IDLE;
	player->reblit = _coler_reblit;
	player->set_state = _coler_set_state;
	player->ready = _ready;
	player->free = _coler_free;
