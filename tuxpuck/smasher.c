...
/* unused?
static void _smasher_serve(AIPlayer * player, Uint32 time) {
...
static void _smasher_set_state(AIPlayer * player, Uint8 state) {
	player->state = state;
...
AIPlayer *smasher_create(Pad * pad, Puck * puck) {
	...
	player->sdl_image = video_create_png_surface(smasher_png, NULL);
	...
	player->puck = puck;
	player->speed = 2.0;
	player->hit_power = 3.8;
	player->state = PLAYER_STATE_IDLE;
	player->reblit = _smasher_reblit;
	player->set_state = _smasher_set_state;
	player->ready = _ready;
	player->free = _smasher_free;
