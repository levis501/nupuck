...
/* externals */
extern unsigned char tin_png[];

/* statics */
static char _name[] = "Tin";
/* unused? static Uint8 _serve_state = 0, _is_ready = 1;*/
static Uint8 _is_ready = 1;

/* functions */
static void _tin_free(AIPlayer * player) {
	SDL_FreeSurface(player->sdl_image);
	free(player);
}

static Uint8 _tin_reblit(AIPlayer * player, Uint32 time) {
	switch (player->state) {
...
/* unused?
static void _tin_serve(AIPlayer * player, Uint32 time) {
...
static void _tin_set_state(AIPlayer * player, Uint8 state) {
	player->state = state;
...
AIPlayer *tin_create(Pad * pad, Puck * puck) {
	...
	player->sdl_image = video_create_png_surface(tin_png, NULL);
	...
	player->puck = puck;
	player->speed = 0.9;
	player->hit_power = 1.2;
	player->state = PLAYER_STATE_IDLE;
	player->reblit = _tin_reblit;
	player->set_state = _tin_set_state;
	player->ready = _ready;
	player->free = _tin_free;
