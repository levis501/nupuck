...
void board_init(void) {
	...
	_rect.w = 0;
	_rect.h = 0;
	_pad_1 = pad_create(FIRST_PLAYER);
	_pad_2 = pad_create(SECOND_PLAYER);
	_puck = puck_create();
	_turn = 1;
	glass_init();
	_coeff[0] = 80 * (BOARD_BOTTOM * BOARD_TOP) / (BOARD_BOTTOM - BOARD_TOP);
	_coeff[1] = 40 * (BOARD_BOTTOM + BOARD_TOP) / (BOARD_BOTTOM - BOARD_TOP);
	_coeff[2] = 80 * (1.0 * BOARD_LEFT / BOARD_RIGHT) / (1.0 - BOARD_LEFT
			/ BOARD_RIGHT);
	_coeff[3] = 40 * (1.0 + BOARD_LEFT / BOARD_RIGHT) / (1.0 - BOARD_LEFT
			/ BOARD_RIGHT);
